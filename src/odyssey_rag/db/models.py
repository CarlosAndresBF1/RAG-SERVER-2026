...
class Document(Base):
    ...
    integration: Mapped[str] = mapped_column(String(100), default="odyssey", nullable=False)
...
class SourceTypeRule(Base):
    """Admin-managed custom source type detection rule.

    DB-backed rules supplement the hardcoded regex rules in the ingestion
    pipeline.  Rules are ordered by ``priority`` (lower = checked first)
    and can be soft-deleted via ``is_active``.
    """

    __tablename__ = "source_type_rule"
